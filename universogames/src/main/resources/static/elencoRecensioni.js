$(document).ready(function() {
	...
	function getReviews() {
		...
		$(".elenco").html('');
		$.get(`posts/list/reviews/9/${npage}`, function(res) {
                ...
                <div class="card-body">
                   <h1 class="intro-text">${res[i].title}</h1>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                      <div class="btn-group">
                        <button type="button" class="btn btn-sm btn-outline-secondary button_dettaglio" data-id='${res[i].id}' onclick="document.location='recensione.html'">Leggi</button>
                      </div>
                        <small class="text-muted">Voto: ${res[i].score}</small>
                    </div>
                </div>
              </div>
            </div>`).appendTo('.elenco');
